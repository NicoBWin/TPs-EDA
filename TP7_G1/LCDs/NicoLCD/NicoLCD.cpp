...
******************************************************************************/
#define BACKG_FILE	"LCDs/NicoLCD/resources/backgroundN.jpg"
#define FONT_FILE "LCDs/NicoLCD/resources/fontN.TTF"
...
DispN::DispN() {
    ...
    al_set_window_title(displayN, "Nico's LCD");
    ...
    fontN = al_load_ttf_font(FONT_FILE, SIZE_FONT, 0);
    if (!fontN) {
    ...
    updateDisplay();
    NicoError = lcdError("ALL_INIT", "EVERYTHING LOAD WELL", DispOKEY);
}

DispN::~DispN() {
    al_destroy_font(fontN);
    al_destroy_bitmap(backgroundBM);
    al_destroy_display(displayN);
}
...
bool DispN::lcdInitOk() {
    if (NicoError.getErrorCode() == DispOKEY) {
        return true;
    }
    else
        return false;
...
basicLCD& DispN::operator<<(const char c) {
...
void DispN::updateDisplay() {
    ...
    for (int j = 0; j <= MAX_ROW; j++) {
        for (int i = 0; i <= MAX_COL; i++) { //Imprimo todos los caracteres en el display
            ...
            temp[0] = msg[j][i];
            temp[1] = '\0';
            al_draw_text(fontN, al_map_rgb(0, 0, 0), 71 + (23.8 * i), 90 + (40 * j), ALLEGRO_ALIGN_CENTER, temp);
        }
    }
    ...
    higligt[1] = '\0';
    al_draw_text(fontN, al_map_rgb(255, 0, 0), 71 + (23.8 * coord.column), 90 + (40 * coord.row), ALLEGRO_ALIGN_CENTER, higligt); //Imprime la legra a higligtear
